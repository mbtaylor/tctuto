...
\definecolor{pink}{rgb}{1.0,0.75,0.75}
...
\newcommand{\burl}[1]{{\color{blue}\texttt{\url{#1}}}}
\newcommand{\bhref}[2]{\href{#1}{{\color{blue}#2}}}
\newcommand{\tcmanref}[2]
               {\bhref{http://www.starlink.ac.uk/topcat/sun253/#1.html}{#2}}
\newcommand{\stiltsmanref}[2]
               {\bhref{http://www.starlink.ac.uk/stilts/sun256/#1.html}{#2}}
\newcommand{\stiltsttref}[1]{\stiltsmanref{#1}{\texttt{#1}}}
\newcommand{\paragap}{\vspace{1.5ex}}
\newcommand{\entry}[2]{\lab{#1}: ``\texttt{#2}''}

% Don't use \entry{a}{b} if the second parameter b has underscores,
% because they won't be cut'n'pasteable from the PDF.
% Instead you have to write manually \pentry{a} ``\verb|b|''.
% This is annoying and there may be a more elegant solution,
% but I didn't find it so far.
\newcommand{\pentry}[1]{\lab{#1}: }

\begin{document}
\raggedright

\title{Tutorial: Exploring Gaia data with TOPCAT and STILTS}
\author{Mark Taylor, University of Bristol, UK \\
        \texttt{m.b.taylor@bristol.ac.uk}}
\date{\vspace*{-4ex}}  %  git date used in footer instead

\maketitle

\begin{description}
  \item[TOPCAT:] \burl{http://www.starlink.ac.uk/topcat/}
                 {\em (version 4.10-2 or later recommended)}
  \item[STILTS:] \burl{http://www.starlink.ac.uk/stilts/}
                 {\em (version 3.4-1 or later recommended)}
  \item[Mailing list:] {\tt topcat-user@jiscmail.ac.uk}
  \item[Version:] {\tt \input{version}}
  \item[License:] CC-BY
                  {\mbox{\vtop{\vskip-2ex\hbox{\includegraphics[height=2ex]
                                                               {CC-BY.pdf}}}}}
\end{description}
\vspace*{-0.4cm}
...
It assumes you have TOPCAT, and hence STILTS, installed.
For best results, you should have the manuals to hand:
\burl{http://www.starlink.ac.uk/topcat/sun253/} and
...
\label{sec:m4-cone}
  ...
  \item Examine the loaded table: look at the
        \mb{tablebrowser_button.png}{Views}{Table Data} and
        \mb{cols_button.png}{Views}{Column Info} windows
        (you can either go through the menus or find the right button
        in the toolbar)
  ...
  \item Play with the plot.  Note overdense regions are coloured darker.
        ...
        The \mb{rescale_button.png}{Plot}{Rescale} button
        is useful to reset the scaling.
  \end{enumerate}
\end{minipage}
...
\subsection{Manage Subsets}
  ...
  \item Create an additional subset that contains the background objects,
        ...
        then invoke the \mb{invert_button}{Subsets}{Invert Subset} action.
...
\label{sec:histo-mean}
  ...
  \item Zoom in horizontally (mouse below X axis, use wheel/CTRL-drag)
        ...
        background sample.
  \item For a more precise result, fit Gaussian to data:
...
% \item Look in the \lab{Service} tab to see information about the service
  ...
  \item Note that syntax errors (including partial or misspelt tables/columns)
        get highlighted \colorbox{pink}{{\tt in red}} --- click
        \ma{warning_button.png}{Parse Errors} button for details.
  \item Hit \lab{Run Query} to run the query;
        if successful a new table will be loaded
  \end{enumerate}
...
We are retrieving all the basic astrometric parameters
(enough for the 6-d phase space) and some photometry.
\paragap
...
TOPCAT's \tcmanref{jel}{\em expression language\/} can help.
\vspace*{3ex}
...
\begin{minipage}[t]{11cm}
  ...
  \item Open the \mb{fx_button}{Help}{Available Functions} browser to see
        what functions TOPCAT provides
        (they are \tcmanref{generalJel}{listed in the manual} too).
  \item Look under the \lab{Gaia} entry to see astrometry-specific items
  \item We will use the {\tt astromUVW(astrom6)} and maybe {\tt icrsToGal}
        ...
        you can often just copy and paste
  ...
  \item Look at the new column in the
        \mb{tablebrowser_button.png}{Views}{Table Data}
...
\begin{minipage}[t]{11cm}
  ...
  \item Plot points in 3-d space:
        ...
        \\
        \textit{\textbf{Note:} TOPCAT versions 4.9 and earlier don't have the
          \lab{Coordinates} control; in that case enter
          \texttt{uvw[0]}, \texttt{uvw[1]}, \texttt{uvw[2]} in the
          \lab{X}, \lab{Y}, \lab{Z} fields.}
  \item Select \entry{Mode}{\buttimg{MODE_DENSITY.png}density}
        in the \lab{Form} tab.  You can change the colour map to taste
        using the \lab{Shader} selector.
  ...
  \item Navigate so only the objects in the overdense region
        ...
        are visible inside the wireframe --- these should be the Hyades,
        which all have similar 3D velocities.
  \item Use \mb{spoints5.png}{Subsets}{Subset From Visible} action,
...
\subsection{View positions}
  ...
  \item Use the \lab{Subsets} tab to make sure that \lab{hyades}
        and background objects (\lab{All}) are plotted in different colours.
        You might want to set \entry{Shading Mode}{flat} and
        fiddle with marker size and shape for clarity (in the \lab{Form} tab).
  ...
  \item Leave any plot windows open for later.
  \end{enumerate}
...
\subsection{Colour-magnitude diagram}
  ...
  \item Plot a colour-magnitude diagram:
        ...
        \pentry{X} ``\verb|bp_rp|'', \pentry{Y} ``\verb|g_abs|''.
...
\subsection{Investigate outliers}

There are some sources sitting off the main sequence.
We will try to find more information about these.

\begin{enumerate}
\item In the \lab{Subsets} tab hide the \lab{All} subset
      leaving just the \lab{hyades} visible.
\item Now click on an outlier.  A little round cursor will appear on it.
      The same cursor will identify that source in any other plots it
      appears in, e.g.\ the sky plot from section \ref{sec:hy-sky}.
      In some cases you can see by sky position that the source is a
      non-member.
\item Open the \mb{tablebrowser_button.png}{Views}{Data Window}
      and see that the relevant row is highlighted when
      you click on a plotted point.
      This works the other way round too, as long as the point is visible.
\item You can make all sorts of other things happen when you click on
      a plotted point (or row in the table window) as well.
      Open the \mb{activate_butt}{Views}{Activation Actions} window
      to see how these are controlled.
\item Check the checkbox for \lab{Display HiPS Cutout}
      and click again on a plotted point --- a little window
      should pop up with a sky image centered on that source.
      Click on the \lab{Display HiPS Cutout} item to configure it better,
      e.g.\ \entry{Field of View}{1 arcmin},
      or select a different \lab{HiPS Survey} to supply the imagery,
      then click again on a plotted point to see it working.
\item If Aladin Desktop is running, try the
      \lab{Send Sky Coordinates} option too which uses a
      communication protocol called SAMP;
      Aladin should re-center on the source you've clicked on.
      Aladin Lite can do the same thing too, but a bit of extra
      setup is required to make it listen to SAMP
      (in Aladin Lite:
       \buttimg{aladinsettings_button.png}~{\bf Settings}$\mid$
       \buttimg{aladinsamp_button.png}~{\bf SAMP}$\mid$
          {\bf Connect to SAMP Hub})
...
\begin{minipage}[t]{7.5cm}
  \vspace*{-0.5cm}
  \winfig{width=7.5cm}{hy_sky.png}
  \\[0.5cm]

  \winfig{width=7.5cm}{hy_cmd.png}
  \\[0.5cm]

  \winfig{width=7.5cm}{hy_activate.png}
\end{minipage}
...
We match these positions with positions in the main Gaia DR3 catalogue
at J2016.0.
...
\begin{minipage}[t]{11cm}
  ...
  \item Enter in \entry{Keywords}{Gouliermis}
...
\begin{minipage}[t]{11cm}
  ...
  \item Fill in the fields:\\
        ...
        \pentry{Input Table} ``\verb|J_ApJS_166_549_table2|'' \\
        (or whatever the HST table is called in TOPCAT) \\
        \pentry{RA column} ``\verb|_RAJ000|'',
        \pentry{Dec column} ``\verb|_DEJ2000|''  \\
        (should be filled in automatically) \\
        \entry{Radius}{1} (arcsec) \\
        \entry{Find Mode}{All} \hspace{1em}{\em $\longleftarrow$ Important!}
  \item Hit \lab{Go};
        within a few seconds, it should inform you that a new table has been
        loaded, with about 24\,000 rows.
  \item Look at the columns of the new table
        (all 10 HST followed by all $\sim$140 Gaia)
...
\begin{minipage}[t]{11cm}
  ...
  \item Plot the HST observations:
        \pentry{Table} ``\verb|J_ApJS_166_549_table2|'',
        \pentry{Lon} ``\verb|_RAJ2000|'', \pentry{Lat} ``\verb|_DEJ2000|''
  \item Overplot the actual matches.
        ...
        \pentry{Lon(1)} ``\verb|_RAJ2000|'',
        \pentry{Lat(1)} ``\verb|_DEJ2000|'' (HST) \\
  ...
  of HST sources on the length scale of the match radius.
...
\begin{minipage}[t]{11cm}
  ...
  \item Add new columns giving RA/Dec discrepancies between HST and Gaia
        ...
        \entry{Name}{deltaRa},
        \pentry{Expression} ``\verb|3600*(RAdeg - _RAJ2000)|'',
        \entry{Units}{arcsec} \\
        \entry{Name}{deltaDec},
        \pentry{Expression} ``\verb|3600*(DEdeg - _DEJ2000)|'',
        \entry{Units}{arcsec}
  \item Use \mb{planeplot_button.png}{Graphics}{Plane Plot} window, \\
        plot \entry{X}{deltaRa}, \entry{Y}{deltaDec}
  \item Identify overdense region, select by drawing a blob
        as in section~\ref{sec:blob},
...
\begin{minipage}[t]{11cm}
  ...
  \item Plot the HST (from VizieR) and Gaia (from Cone Search)
        datasets on the sky:
...
\item Unlike TOPCAT, you can't bluff your way through STILTS
      ...
      Find the manual \burl{http://www.starlink.ac.uk/stilts/sun256/}
      or maybe just google for ``stilts''
      and go to the \lab{Documentation} section.
\item Make sure you have it installed.
      Running ``{\tt stilts}'' on its own may work; if not then
      ``{\tt java -jar stilts.jar ...}'' should do.
...
\item Investigate the STILTS {\tt cone} command;
      see the \stiltsmanref{cone}{\texttt{cone} documentation}
...
\item Putting those together, we need to run
      ...
      (you can write e.g.\ {\tt m4.vot} if you prefer a different format).
      \paragap
\end{enumerate}
...
A good tip is to avoid spaces where you don't need them, e.g.\ in expressions.
...
``\stiltsttref{select}'' (retain only some rows) or
``\stiltsttref{addcol}'' (add a new column).
Filters are specified by adding using zero
or more ``{\tt cmd=}'' parameters on the command line,
and documented in
\stiltsmanref{filterSteps}{Section 6.1: ``Processing Filters''}
of the STILTS manual.
\paragap

The default {\em output mode\/} is write to a file,
but other options such as
``\stiltsmanref{mode-meta}{\texttt{meta}}'' (display column metadata),
``\stiltsmanref{mode-count}{\texttt{count}}'' (count rows) and
``\stiltsmanref{mode-stats}{\texttt{stats}}'' (calculate mean, st.dev etc)
are also available.
Select non-default output modes using the ``{\tt omode=}'' parameter
on the command line, documented in
\stiltsmanref{outModes}{Section 6.4: ``Output Modes''} of the manual.
...
\item For convenience, write a new table with only a few of the columns.
      ...
      which tends to clutter up the screen output.
      \preverb
      \begin{verbatim}
...
\begin{minipage}[t]{7cm}
  \winfig{width=7cm}{stilts_plot.pdf}
\end{minipage}
...
\textit{\textbf{Note:} TOPCAT versions 4.10 and earlier don't have
  the STILTS interoperability features described here for Cone Search etc,
  only for plotting windows.}
\paragap
...
\begin{minipage}[t]{12cm}
  ...
  \item Note that changing selections and controls in the Cone Search window
        changes the settings of the displayed STILTS command.
...
Explore \bhref{http://www.starlink.ac.uk/stilts/sun256/}{the manual}! \\
Or try to replicate the other exercises using STILTS.
...
\begin{minipage}[t]{11cm}
  ...
  \item Choose \entry{Mode}{Asynchronous}
        ...
        so a synchronous query might time out or overflow.
...
\label{step:hrd-data}
    ...
    they are nominally within 100\,pc, and have $\varpi$ (parallax), BP and RP
...
\begin{minipage}[t]{11cm}
  ...
  \item Add a new column calculating absolute G magnitude, using parallax:
        ...
        \pentry{Name} ``\verb|g_abs|'', \\
        \pentry{Expression}
                ``\verb|phot_g_mean_mag + 5*log10(parallax/100)|'', \\
        \entry{Units}{mag}
        \label{item:calc-absmag}
  \item Make a \mb{planeplot_button.png}{Graphics}{Plane Plot}, with
        \pentry{X} ``\verb|bp_rp|'' ($BP-RP$ colour),
        \pentry{Y} ``\verb|g_abs|''
...
\begin{minipage}[t]{11cm}
  ...
  \item Open a new \mb{planeplot_button.png}{Graphics}{Plane Plot}, with
        \pentry{X} ``\verb|bp_rp|'' ($BP-RP$ colour),
        \pentry{Y} ``\verb|phot_bp_rp_excess_factor|''.
  \end{enumerate}
  ...
  \begin{enumerate}
  \setcounter{enumi}{1}
  \item Click the \mb{poly2.png}{Subsets}{Draw Algebraic Subset} action
  \item Select \entry{Shape mode}{Below} in the popup
  ...
  \item When you're done, click on the \buttimg{unpoly2.png} button again.
        This time you get a numeric characterisation of the region
        you've marked out;
        this could e.g.\ be quoted in a paper.
  ...
  \item Create a new subset combining the two: \\
        ...
        \pentry{Expression} ``\verb|astrom_ok && photom_ok|''
...
\begin{minipage}[t]{11cm}
  \raggedright
  The photometry and astrometry in Gaia DR3 is so good that plotting
...
% \item Unlike TOPCAT, you can't bluff your way through STILTS
%       by pointing'n'clicking; you'll need some documentation.
%       Find the manual at \burl{http://www.starlink.ac.uk/stilts/sun256/},
...
\bibitem{2005ASPC..347...29T}
   ...
   ASP Conf.\ Ser.\ 347, 29 (2005),
   {\tt 2005ASPC..347...29T}
\bibitem{2006ASPC..351..666T}
   M.B.Taylor,
   ``STILTS --- A Package for Command-Line Processing of Tabular Data'',
   ASP Conf.Ser.\ 351, 666 (2006),
   {\tt 2006ASPC..351..666T}
\end{thebibliography}
...
If you use TOPCAT or STILTS in your work, please consider citing
\cite{2005ASPC..347...29T} or \cite{2006ASPC..351..666T}.
