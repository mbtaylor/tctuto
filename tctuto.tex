...
\label{sec:m4-cone}
  ...
  \item There are a few options, that should mostly give similar results.
        {\tt eDR3 lite Cone} is a good choice.
  ...
  \item Hit \lab{OK};
        ...
        (if download is too slow, cancel and try with \entry{Radius}{0.1},
        which is about 20\,000 rows,
  ...
  \item Play with the plot.  Note overdense regions are coloured darker.
        ...
        for navigation help; details are different for different
        plot types.
  \end{enumerate}
\end{minipage}
...
\begin{minipage}[t]{8cm}
  \begin{picture}(8,0)
  \put(0,-8){\winfig{width=8cm}{m4_pmplot.png}}
  \end{picture}
\end{minipage}
...
\label{sec:histo-mean}
  ...
  \item Zoom in horizontally (mouse below X axis, use drag and wheel/CTRL-drag)
        to read off the peak/average parallax
...
\begin{minipage}[t]{11cm}
  ...
  with radial velocities (only about 7 million DR2/EDR3 sources have RV)
